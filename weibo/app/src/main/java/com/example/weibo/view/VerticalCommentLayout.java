package com.example.weibo.view;
...
public class VerticalCommentLayout extends LinearLayout implements
    ...
    private List<Reply> mCommentBeans;
    ...
    public void addCommentsWithLimit(List<Reply> commentBeans, int limit, boolean more) {
        ...
        for (int i = 0; i < showCount; i++) {
            ...
            Reply commentBean = commentBeans.get(i);
            if (childView == null) {
    ...
    public void addComments(List<Reply> commentBeans) {
        if (commentBeans == null) return;
        this.mCommentBeans = commentBeans;
        int oldCount = getChildCount();
        if (oldCount <= 0) {
     ...
     */
    public void updateTargetComment(int position, List<Reply> commentBeans) {
        int oldCount = getChildCount();
        for (int i = 0; i < oldCount; i++) {
            if (i == position) {
                View childView = getChildAt(i);
                Reply bean = commentBeans.get(i);
     ...
     * 創建Comment item view
     */
    private View makeCommentItemView(Reply bean, int index) {
     ...
     * 添加需要的Comment View
     */
    private void addCommentItemView(View view, Reply bean, int index) {
     ...
     */
    private void updateCommentData(View view, Reply bean, int index) {
        bindViewData(view, bean);
    }

    private View makeContentView(Reply content, int index) {
        View view = View.inflate(getContext(), R.layout.item_comment_single_child_new, null);
        bindViewData(view, content);
        return view;
    }

    private void bindViewData(View view, final Reply content) {
        ...
        Glide.with(iv_header.getContext()).load(ApiConfig.PROFILE + content.getProfileURL()).into(iv_header);
        iv_like.setImageResource(content.getIsLike() == 0 ? R.drawable.icon_topic_post_item_like : R.drawable.icon_topic_post_item_like_blue);
        ...
        if (content.getIsReply() == 0) {
            ...
            SpannableString stringBuilder = makeReplyCommentSpan(content.getFname(), String.valueOf(content.getFid()), content.getText());
        ...
        tv_user_name.setText(content.getName());
...
//                    Toast.makeText(CommentApplication.getApplication(), atSomeone + " id: " + id, Toast.LENGTH_LONG).show();
    ...
    public interface CommentItemClickListener {
        ...
        void onItemClick(View view, Reply bean, int position);

        void onLikeClick(View layout, Reply bean, int position);
